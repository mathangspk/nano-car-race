...
## Summary of Changes
- **Joystick Direction Correction**: Negated `moveSpeed` in `CarApplication.cpp` (`moveSpeed = -moveSpeed * throttleScale;`). This corrects the reversed forward/backward movement mapping so pushing UP drives the car forward (`tiến`) and pulling DOWN drives the car backward (`lùi`).
