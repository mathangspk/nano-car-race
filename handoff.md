...
## Summary of Changes
- **Crossed Joystick Axes Correction**: Swapped `TURN_CHANNEL_PIN` and `MOVE_CHANNEL_PIN` back in `Config.h` (TURN to pin 2, MOVE to pin 3) to fix crossed joystick axes where pushing forward/backward was incorrectly affecting the turn channel.
- **Joystick Direction Correction**: Removed the negation on `moveSpeed` in `CarApplication.cpp` (`moveSpeed = moveSpeed * throttleScale;`). This corrects the reversed forward/backward movement mapping so pushing UP drives the car forward (`tiến`) and pulling DOWN drives the car backward (`lùi`).
