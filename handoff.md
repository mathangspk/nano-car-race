...
## Summary of Changes
- **Calibration Logging**: Added periodic (500ms) Serial logging in `CarApplication::update()` to print signals validation status, raw RC pulse widths (`turnPulse`, `movePulse`, `throttlePulse`), mapped speed components, and final left/right motor output speeds. This helps calibrating the RC channels.
